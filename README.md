...
- Grounded-or-refuse: the model answers from provided material or says it can't — in both modes. **The refusal applies to questions, not to instructions**: the grounding prompt distinguishes *"what does this note say about X"* (refuse if absent) from *"turn this into flashcards"* (carry out, using only what is there). Without that split a small model treats an instruction as a lookup — asked for flashcards it went looking for flashcards inside the note, found none, and refused.
