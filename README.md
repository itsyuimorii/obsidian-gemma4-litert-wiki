...
- **Save answers back** — every reply has a save-to-wiki action (same review gate), so an answer you want is kept in `answers/` instead of vanishing with the chat. Neither `answers/` nor `chats/` is retrieved as grounding: **an answer is output, not material.** When it was grounded, what it says already lives in the cards it came from; when it was not, it is the model's own knowledge — and either one, fed back, would come back months later cited to your own wiki and indistinguishable from something you wrote. To make an answer count as material, put it in a note and ingest that.
...
- **Saved answers are retrieved, but ranked below the pages they came from.** Karpathy's gist says explorations should compound and says nothing about trust, which is the poisoning loop — a wrong or stale answer re-entering context with the same weight as the notes. Answers arrive in their own labelled block ("where they disagree with a page, trust the page"), they record what each page looked like when they read it so the review board can flag them when it changes, and a skill's output is marked `derived` and never retrieved at all, being a rendering of the material rather than a synthesis.
...
- Grounding means **the material is the subject**, not the only vocabulary the model may use — and the two modes draw that line differently. **Wiki mode is grounded-or-refuse**: a claim about a body of pages you cannot eyeball is where invented detail does real damage, and it is the mode whose Sources row is load-bearing. **This note mode explains**: the note is open in front of you, so asking what a term in it means is a question about the note, and the answer separates *what the note states* from *what the term means* rather than refusing.
...
Either way: answers stream in from a model running inside Obsidian's own process, every answer ends with a deterministic **Sources** row (clickable — listed by the plugin, not left to the model to cite), and per-message **copy / regenerate / save-to-wiki** actions. Wiki mode refuses honestly when the retrieved pages don't answer the question; This note mode will explain a term the note uses instead of repeating the note back at you, keeping what the note says separate from what the term means. A **+** button attaches additional notes as removable context pills; a **⚡ skills** menu runs canned single-task prompts (*Quiz*, *Flashcards*, *Find gaps*) against the open note.
