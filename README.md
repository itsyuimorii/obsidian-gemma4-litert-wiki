...
**Gemma 4 E4B builds [Andrej Karpathy's LLM wiki](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f) out of the notes you already have** — a summary card for each note, an index, concept pages over the clusters that emerge, tags drawn from one shared vocabulary instead of whatever each note happened to invent, and links between notes that turn out to be related. Everything is plain Markdown, in your vault.

Then **chat with one note or the whole thing**, grounded in what you actually wrote, with sources listed by the plugin rather than generated by the model. Find gaps and contradictions. Turn any note into quizzes and flashcards.
