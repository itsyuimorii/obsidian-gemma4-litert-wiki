...
- **Save answers back** — every reply has a save-to-wiki action (same review gate), so an answer you want is kept in `answers/` instead of vanishing with the chat. It is **not** retrieved as grounding: an answer is output, not material. When it was grounded, what it says already lives in the cards it came from; when it was not, it is the model's own knowledge — and either one, fed back, would come back months later cited to your own wiki and indistinguishable from something you wrote. Want it to count? Write it into a note of your own and ingest that.
