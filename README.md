...
- **Saved answers are never retrieved.** Karpathy's gist says explorations should compound and says nothing about trust, which is the poisoning loop — a wrong or stale answer re-entering context with the same weight as the notes. So answers live in `answers/`, outside the retrieval set entirely: kept to re-read, with frontmatter recording which model wrote them and from which sources. The one route back in is deliberate and human — save an answer into your own notes, judge it, and ingest it like anything else you wrote.
