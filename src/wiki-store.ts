...
const FOLDER_READMES: Array<[() => string, () => string]> = [
  ...
  [
    ...
    `# ${_wikiDir}\n\n` +
      ...
      `> | **Ask your wiki** | The other half of the same panel. Reads \`index.md\` to pick the pages worth opening, then answers from those, citing them. A question about the *collection* — *Find connections*, *Added this week?* — grounds in **every page instead**, because "what links my pages" is not a question retrieval can find an answer to. |` + `\n` +
      ...
      `> | The chips above it | Three one-press starters, fixed — the row you learn is the row you keep. In Wiki mode: *Scan a folder*, *Find connections*, *Added this week?* **While a scan is running, the first one becomes *Stop scan*.** **A chip with a small icon does something; a chip without one asks something.** The ones that do are always behind their own preview. |` + `\n` +
