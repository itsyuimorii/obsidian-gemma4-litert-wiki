...
README when a version is released, and the whole opening was rewritten after
1.0.9 shipped.

- **The README opens on the reader's problem rather than on the runtime.** The
  first line named Gemma 4 E4B, LiteRT-LM and WebGPU — the right answer for
  someone already comparing local-AI plugins, and the wrong one for everyone
  else. It now opens on the vault full of things you clipped and never
  reopened, and the architecture closes the section instead of starting it.
- **What it does is six scannable lines instead of two paragraphs.** The store
  truncates the Overview behind "Show more", so the first screen is most of
  what a visitor reads. Every claim survived the move, including the two that
  matter most: sources are listed by the plugin rather than generated by the
  model, and tags come from one shared vocabulary rather than whatever each
  note invented.
- **The head is the logo, the title, and the sentence.** The poster above the
  title was the same image the store already shows in its gallery, so the
  listing page opened with it twice; it is gone, and the logo is back at 96px.
- **One sentence across the store, the README and the manifest.** The centred
  tagline still opened on the runtime, three lines above a section that opens
  on the backlog. Both now say what the store listing says.
- **The Japanese README says the same things in the same order**, and no longer
  claims the plugin is awaiting review — it has been in the store since 1.0.8.
